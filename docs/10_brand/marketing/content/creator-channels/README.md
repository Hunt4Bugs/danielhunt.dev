---
id: content.creator-channels.readme
kind: note
domain: content
status: active
version: 1
class: "10"
collection: content
owner: Daniel Hunt
created: 2026-08-28
updated: 2026-08-28
facets:
  - marketing
related:
  - ../concepts.md
  - ../_patterns/creator-channel.md
  - ../creators/README.md
  - ../../../channels/README.md
sources:
  - ../../../ONTOLOGY.md
---

# Creator Channel records

You are in the instance directory for `content.creator-channel`. Store one specific account a
Creator holds on a Channel platform type here. A Creator Channel is not the platform type itself —
that registry belongs to [Channels](../../../channels/README.md).

- What a Creator Channel *is* (definition, relationships, constraints, entity contract):
  [`concepts.md`](../concepts.md#creator-channel).
- What a Creator Channel document must *contain* (required sections):
  [`_patterns/creator-channel.md`](../_patterns/creator-channel.md).
- Related instance directories: [Creators](../creators/README.md) (owns each Creator Channel).
