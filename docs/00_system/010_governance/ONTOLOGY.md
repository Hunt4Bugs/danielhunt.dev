...
This is the Brand-root model: it owns entity definitions, relationships, and taxonomy ownership across the whole personal-brand corpus. [`content/domain.md`](../../40_content/README.md) and [`content/concepts.md`](../../40_content/concepts.md) are Content's domain-scoped instantiation of it under the [Domain Documentation Protocol](DOMAIN_PROTOCOL.md) — they cross-reference the definitions, relationships, and constraints below rather than re-authoring them, and add the field-level entity contracts (§10) each Content concept's persisted instances require. Content is the first Brand context fully modeled under that protocol. It briefly graduated to a root-level, unnumbered directory before settling at its own numbered class, `docs/40_content/` (per [`LIBRARY_MAP.md`](LIBRARY_MAP.md)). Content is now the only Brand context: Identity, Strategy, Audience, Marketing, Offers, Assets, Channels, and Relationships were all retired as standalone domains through September 2026, their real content folded into Content rather than deleted.

## Bounded contexts

`Brand` is the root domain, and Content is now its only context. Content — modeled independently at [`docs/40_content/`](../../40_content/README.md) — contains Knowledge, Source, Topic, Blueprint, Publication, Series, Channel, Theme, Audience Segment, Service Offer, Product Offer, Asset, Template, Measurement, Insight, Creator, Creator Channel, Review, Motif, Workflow, Work Item, and Taxonomy. Script and Script Template are Content Asset subtypes.
...
| Creator | An identity-bearing entity Daniel records and relates to — himself, an external creator monitored for competitor, inspiration, peer, or reference intelligence, or a general person or organization worth tracking. |
...
- Series can group zero or many Topics and Publications.
...
Controlled vocabularies remain with the context that owns their meaning. This ontology owns the taxonomy definitions below; [Content](../../40_content/README.md) owns Theme, Audience Segment, and Channel, alongside every other Content-owned vocabulary. A governed addition updates its authoritative owner and is not selectable before approval.
...
| Creator Relationship | Creator | Competitor, Inspiration, Peer, Reference, Self | Our stance toward the Creator. Multi-valued — a Creator may hold more than one Creator Relationship simultaneously (for example, both Competitor and Inspiration). `Self` marks Daniel Hunt's own record. |
...
Do not model releases, claims, experiments, projects, or a full CRM system as first-class records yet. Promote them only when recurring work requires ownership, lifecycle, or analysis beyond a simple reference.

Creator (`content.creator`) covers Daniel Hunt himself (`Self`), external creators monitored for
competitor, inspiration, peer, or reference intelligence, and general people or organizations
worth recording — the boundary formerly reserved for a separate Relationships context has been
absorbed into Creator. Promote a narrower People/Organization concept only if Creator's fields
(`creator_type`, and `relationships` as Competitor/Inspiration/Peer/Reference/Self) stop fitting a
recurring need.
