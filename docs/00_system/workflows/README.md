...
- [Create Domain](create-domain.md) — the root-level procedure for standing up a new
  protocol-conformant domain from a name and an intent. This is the workflow that originally
  produced `content`'s scaffold and, in the same pass, every other Brand context then modeled
  (Identity, Strategy, Audience, Marketing, Offers, Assets, Channels, Relationships, Analytics).
  Identity, Marketing, Channels, and Relationships were later retired — their scope was absorbed
  into Content's Creator entity (identity, and Channels' registry) or dropped outright (Marketing's
  unused Campaign/Distribution/Growth/Funnel scope); the current Brand contexts are Strategy,
  Audience, Offers, Assets, Analytics, and Content.
