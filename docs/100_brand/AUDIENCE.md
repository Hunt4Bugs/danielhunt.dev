...
Companion to [`IDENTITY.md`](IDENTITY.md). IDENTITY defines who the brand is. AUDIENCE defines who the brand is for, what they are stuck with, what content addresses, and what credibility justifies speaking on it.
...
The Brand Statement, the brand-level contrarian, the product-level contrarian, and the bridge sentence all live in IDENTITY.md. They are the touchstones this document writes downstream from.
...
| 11 | Want to travel but waiting until I "make it." | Don't defer the life. Travel is a tool for being alive, not a reward. Build the work around travel by batching production during trips. The plan accommodates the life, not the other way around. | Twice-yearly Thailand and Asia rotation is built into OPERATING.md, with each trip batched as 3 to 4 episodes of footage. Travel is structural, not pasted on. |
...
**Intentionally empty for Phase 0.** Brand-process work (DaVinci practice, on-camera comfort, frame-by-frame study of Lindhardt and Dalen) is happening per OPERATING.md, but is kept backstage as content. Risk: meta-content about learning production craft can compound or can read self-indulgent. The reference set (Dalen specifically) does not do this. Revisit at Month 6 if Phase 0 craft has progressed enough to have something honest to teach.
...
Each row connects to a contrarian or principle in IDENTITY.md. Each row would force a hard no on a specific tempting decision the brand might otherwise drift toward.
...
**On the bench (not in ship list):** Polished or luxury creator aesthetic versus memory-not-photoshoot. Already covered by the visual rule in [`VISUAL.md`](VISUAL.md) and the aesthetic tie-break in IDENTITY.md. Swap into the table only if the polish-aesthetic tension becomes a recurring decision-pressure point.
...
- **Brand Statement, brand-level contrarian, product-level contrarian, bridge sentence:** [`IDENTITY.md`](IDENTITY.md).
- **Editorial rules and capture targets:** [`OPERATING.md`](OPERATING.md).
